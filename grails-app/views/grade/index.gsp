		...
		<div id="list-grade" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="grade.track.label" default="Track" /></th>
				...
				<g:each in="${gradeInstanceList}" status="i" var="gradeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${gradeInstance.id}">${fieldValue(bean: gradeInstance, field: "grade")}</g:link></td>
					
						<td>${fieldValue(bean: gradeInstance, field: "track")}</td>
					
						<td>${fieldValue(bean: gradeInstance, field: "user")}</td>
